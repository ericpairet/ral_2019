function model = initRBFTimeBased(Data, model)
...
TimingSep = linspace(min(Data(1,:)), max(Data(1,:)), model.nbRBF+1);

for i=1:model.nbRBF
	idtmp = find( Data(1,:)>=TimingSep(i) & Data(1,:)<TimingSep(i+1)); % time idx
